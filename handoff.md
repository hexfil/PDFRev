...
>
> 测试基线：Rust 单元 19 项 + 真实文档端到端 1 套 + 界面自检 110 项，全部通过。
...
**自检新增 7 项**（96 → 103）：
  ...
  - 前端已接原生拖放钩子
### 5.30 插入另一个 PDF 后内容乱码 —— 页面对象必须「重新编号 + 改写引用」

**现象**：用户把两个真实文件放进 `src-tauri/examples\`，用
`VPEg-测试用.pdf`（60 页）插入 `1818090690.pdf`（4 页）后，
**插入的那 4 页显示成乱码 / 空白**（原有 60 页正常）。

**根因**（已用探针确认）：`insert_pdf` 里旧的对象拷贝是这么写的——

```rust
// 旧实现（错的）
if dst.get_object(id).is_err() {
    dst.objects.insert(id, obj);   // 沿用源文档的编号
}
```

两个 PDF 的对象编号是**各自独立分配**的，几乎必然撞车。实测这一对文件：

| | `/Contents` | `/Resources` |
|---|---|---|
| 目标 `VPEg-测试用.pdf` 第 1 页 | `6 0 R` | `7 0 R` |
| 待插 `1818090690.pdf` 第 1 页 | `7 0 R` | `4 0 R` |

目标文档里 `7 0 R` 已经是一个**字体字典**，于是插入页的
`/Contents -> 7` 被「编号已占用，跳过」这一句直接丢掉，
最终指向那个字体字典 —— 解析器拿不到内容流，页就渲染成空白 / 乱码。
页数是**对的**（`/Kids` 插进去了），所以只数页数的测试全绿也照样漏掉。

**为什么测试没早发现**：`make_pdf()` 生成的两个文档编号是固定的
（1=Pages、2=Font、3=第 1 页内容流…），两个 `make_pdf` 撞车时**类型恰好
也一一对应**，撞了也看不出问题。实测：13 项单测全绿，用户一插真实文件就乱码。

**修法**（`src-tauri/src/pdfops.rs`）：

1. 删掉 `copy_referenced`，换成 `copy_page_deep` + `copy_object_refs`：
   **每个被拷贝的对象都分配全新编号**，并递归改写对象内部所有指向它们的
   引用。递归用 `HashMap<老编号, 新编号>` 做「先登记编号再递归」，
   既能去重、又能打断页面树 `Parent` 那种自引用环。
2. 覆盖到嵌套引用：数组、字典、**流字典**里的引用都会改写。
   旧实现只看页面字典的第一层，图片 XObject / 字体描述符里更深一层的
   引用全漏（这个是隐患，不是本次现象的成因）。
3. `/Annots` 保留（表单域、链接、高亮都在这），里面的 `/P` 指回本页时
   会命中 map 的正向映射自动改到新页号；只丢掉 `/Parent`。
   插入完成后统一补上新的 `/Parent`（页面树是平的）。

**验证**（三层都加了对应用例）：

- **Rust 单测 19 项**（14 → 19），新增 5 项：
  - `insert_keeps_page_content_when_ids_collide` —— 核心回归。用
    `make_pdf_padded(n, pad)` 造出「编号重叠且类型不同」的真实撞车局面，
    断言插入后每页 `/Contents` 都能解出非空内容。
    实测把旧实现换回去，这项会红。
  - `insert_survives_shared_font_and_image_resources` —— 字体引用被重写
  - `insert_gives_every_new_page_a_parent`
  - `insert_does_not_break_base_pages`
  - `insert_twice_does_not_duplicate_ids`
- **`cargo run --example vpeg_check`** 新增真实文件回归：直接读
  `examples` 里那两个 PDF 插一次，断言**插入页与源文件逐字节一致**、
  **原有页与插入前逐字节一致**，并写出 `test\insert-out.pdf` 供人工用
  阅读器确认。（找不到文件就跳过并打印提示，CI/别人机器上不会挂。）
- **界面自检 109 → 110 项**：新增「插入的页真的画出了内容（不是空白/指错
  对象）」—— 用 pdf.js 把插入结果和插入源各渲染成缩略图，比**深色像素数**
  （相差 <15%）。原来的「插入后页数 = 原页数 + 2」只数页数，抓不到这个 bug。

**踩坑提醒**：断言「原有页内容流非空」是错的 —— `VPEg-测试用.pdf`
第 57 页本来就是个空白页。要比就得和插入前的原文档逐字节比，
别拿「非空」当健康判据。

**性能**：`copy_page_deep` 是深拷贝，插入大文档会在内存里复制一遍
相关对象。60 页 + 4 页的实测无感；插入整本 62 页大文档（`vpeg_check`
里那个「尾部插入自身」用例）也正常。
...
### 5.31 构建不可逐位复现（每次的 sha256 都不一样，这是正常的）

`cargo build --release` 出的 exe **换一次构建就换一个 sha256**（实测：同一份
源码连续构建两次，`dad04e22…` vs `a5e732f9…`，体积都是 4,723,712 字节）。
原因是链接器会写 PE 头里的时间戳 / 调试目录等元数据，这一块不受源码控制。

**结论**：文档里记录的校验值只对**某一次具体构建的产物**有意义，
验证者拿到的是 Release 上那个文件才对得上；不要拿「本地重新构建一遍，
hash 不一样」当成出错信号。要确认拿到的东西对不对，认体积
（4,723,712 字节）+ 跑一遍 110 项自检更可靠。

首次记录时的 sha256：`A5E732F926DEDCBCACCECF3E6790E48B812CD63D864D59B753446068B941DF14`。
...
### 6.3 界面端到端自检（110 项，跑在真实 WebView2 里）
...
| 剪贴板与插入 | 4 | 写剪贴板、插入 PDF（内存 data）成功且页数 +N、**插入的页真的画出了内容（像素比对，抓乱码 bug）**、插入缺源文件报 pdf.noInsertSource（而不是「目录不存在」） |
...
| 项 | 结果 |
|---|---|
| Rust 单元测试 | 19 项通过，0 失败 |
| 真实文档端到端 | 通过（62 页；删 / 抽 / 转 / 插 / 排序均正确） |
| 源文件完整性 | VPEg.pdf sha256 32045FD8F1ACFD7C 未变 |
| 界面自检 | 110 项通过，0 失败（真实 WebView2） |
| 发布物 | dist\PDFRev.exe 4,723,712 字节（4.50 MB），版本号 0.12.0 |
...
| 插入乱码修复 | `VPEg-测试用.pdf` 插入 `1818090690.pdf`：插入页与源文件逐字节一致，PDF 渲染像素级一致（旧版会渲染成空白/乱码） |
| 版本号与主页 | 版权页显示 PDFRev 0.12.0 与可点击 GitHub 链接；`PDFRev.exe --version` / -V 打印版本号并返回 0 |
| 校验值 | sha256 A5E732F926DEDCBCACCECF3E6790E48B812CD63D864D59B753446068B941DF14（2026-09-23 插入乱码修复构建） |
